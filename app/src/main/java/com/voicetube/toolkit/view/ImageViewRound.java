package com.voicetube.toolkit.view;
...
public class ImageViewRound extends ImageView {
    ...
    public static final String TAG = "ImageViewRound";
    ...
    public ImageViewRound(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
               R.styleable.ImageViewRound, defStyle, 0);

        final int index = a.getInt(R.styleable.ImageViewRound_android_scaleType, -1);
        if (index >= 0) {
            setScaleType(sScaleTypeArray[index]);
        }

        mLeftTopCornerRadius = a.getDimensionPixelSize(
               R.styleable.ImageViewRound_sriv_left_top_corner_radius, 0);
        mRightTopCornerRadius = a.getDimensionPixelSize(
               R.styleable.ImageViewRound_sriv_right_top_corner_radius, 0);
        mLeftBottomCornerRadius = a.getDimensionPixelSize(
               R.styleable.ImageViewRound_sriv_left_bottom_corner_radius, 0);
        mRightBottomCornerRadius = a.getDimensionPixelSize(
               R.styleable.ImageViewRound_sriv_right_bottom_corner_radius, 0);
        ...
        mBorderWidth = a.getDimensionPixelSize(
               R.styleable.ImageViewRound_sriv_border_width, 0);
        if (mBorderWidth < 0) {
            throw new IllegalArgumentException("border width cannot be negative.");
        }

        mBorderColor = a
                .getColorStateList(R.styleable.ImageViewRound_sriv_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
        }

        isOval = a.getBoolean(R.styleable.ImageViewRound_sriv_oval, false);
        a.recycle();
